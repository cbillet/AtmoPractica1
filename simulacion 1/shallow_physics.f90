...
real, allocatable, dimension(:,:) :: h_eq,h_per_s,h_per_c, u_per, h_per, fr
...
real :: xx, yy, dd, yyn, ddn, yys, dds, k0, dd1, j_amp, j_width, jet_lat
real :: v_amp
...
allocate ( h_per_s   (is:ie,js:je) )
allocate ( h_per_c   (is:ie,js:je) )
allocate ( h_per   (is:ie,js:je) )
allocate ( u_per     (is:ie,js:je) )
...
do j = js, je
  do i = is, ie
     xx = (deg_lon(i) - h_lon)/(h_width*1.0)
...
subroutine shallow_physics(Time, dt_ug, dt_vg, dt_hg, ug, vg, hg, fr,   &
...
real, intent(inout),  dimension(is:ie, js:je)    :: fr
